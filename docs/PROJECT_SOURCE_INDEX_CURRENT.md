...
Repository facts and the live task outrank conversational memory. `AGENTS.md` freezes cross-task safety. The task file controls the present allowed surface. The accepted design rebaseline controls the changed product-direction sections; the original GDD seed continues to govern retained sections that are not superseded. The expanded submission roadmap controls the dated execution spine. When sources conflict materially, stop and request GPT Project Sources advisor resolution rather than silently choosing.

Task 0000 did not rewrite either seed. It materialized concise execution rules, current pointers, indexes, and compliance placeholders from them. The 2026-07-22 project-source synchronization adds an accepted successor rather than rewriting historical seeds.