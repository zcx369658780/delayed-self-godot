...
The tracked runtime catalog contains exactly five implemented entries in order: `tutorial_reach_exit`, `tutorial_echo_bridge`, `vertical_slice_delay_3`, `door_one_turn_late`, and `two_keys_one_door`. Their prerequisites form one direct linear chain. The first two HUD modes are `INTRO_MINIMAL` and `GUIDED_ECHO`; all three standard levels use `STANDARD_COMPACT`. Only `two_keys_one_door` is marked final because it is the last currently implemented catalog entry. This is not a narrative ending, complete-flow, candidate-completion, release, or submission claim. No best-turn threshold is recorded.
