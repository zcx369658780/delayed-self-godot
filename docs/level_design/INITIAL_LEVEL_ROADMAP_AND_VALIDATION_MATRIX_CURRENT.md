...
- `PLAN_REQUEST_CHANGES`: GPT rejected Task 0016's one-Plate/one-Door dual-necessity contradiction; no formal file or catalog entry exists.
- `MECHANICAL_NECESSITY_REVISION_READY_FOR_GPT_REVIEW`: Task 0016R retains the bridge sequence and selects a two-Plate A+B grammar with explicit E2/E4 necessity obligations.
...
| 7 proposed | strict load/catalog/progression; solve/replay/restart/hash; `N*=2–32 EXACT`; all-shortest divergence, E2→A/E4→B, A+B, and open-snapshot traces | no-ECHO, E2-only, E4-only, YOU-substitution, overlap, A/B isolation, A-only/B-only, equal-delay, Door/bypass controls | authenticated owner only: explain spacing cause, both-ECHO necessity, and transfer without route recall | can YOU substitute on a Plate, can one ECHO or overlap satisfy A+B, or can the divergence be skipped? |
