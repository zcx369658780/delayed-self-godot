...
- Current gate: Task 0023ZP production Profile bootstrap static implementation
- Active task: `tasks/0023ZP_production_profile_static_bootstrap_implementation_gate.md`
...
Task 0023ZP may implement only the composition/API source surface and focused compile/static contracts. Authorized product changes are the new bootstrap script/scene plus `app_root.gd`, `local_profile.gd`, and `profile_codec.gd`. Adapter, MemoryProgress, RouteRequest, catalog/formal data, UI/gameplay scenes, all historical tests/harnesses and `project.godot` remain frozen.

Task 0023ZP must not access or refresh production Profile metadata/content, access or enumerate the test root, invoke production configuration at runtime, run fixture/file-system behavior, execute the canonical runner/full matrix, aggregate, project main scene, export or build. Validation is limited to the task's new compile/static tests and the named zero-I/O regressions.
...
Do not resume Tasks 0023Y, 0023Z, 0023ZA, 0023ZB, 0023ZE, 0023ZF, 0023ZH, 0023ZI, 0023ZK, 0023ZL, 0023ZJ, 0023ZM, 0023ZN or 0023ZO, and do not reuse their fixtures. Historical blocked tasks 0023, 0023S and 0023U remain closed.
