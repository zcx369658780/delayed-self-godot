...
- DeepSeek MCP is removed and native/automatic DeepSeek providers are forbidden. The sole formal route is the user-scope `Invoke-DeepSeekFileTask.ps1` wrapper for GPT-frozen low-risk bounded work; GPT-5.6 retains planning, allowed paths, review, tests, Git, release and acceptance. Task 0024BAC established a candidate wrapper/worker byte baseline with `route_status = IDENTITY_BASELINE_READY_FOR_GPT_REVIEW`; it is not accepted route identity until GPT adjudicates it. Before any later use, both accepted identities must be verifiable and any drift fails closed without MCP fallback.
