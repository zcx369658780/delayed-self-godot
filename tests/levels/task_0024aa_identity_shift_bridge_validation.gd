...
func _init() -> void:
	_run()
	if failures > 0:
		printerr("TASK_0024AA_IDENTITY_SHIFT_BRIDGE_FAIL failures=%d" % failures)
		quit(1)
		return
	print("TASK_0024AA_RESULT_JSON " + JSON.stringify(_result_payload()))
	print("TASK_0024AA_CONTROL_JSON " + JSON.stringify(summary.controls))
	for record in summary.trace:
		print("TASK_0024AA_TRACE_EVENT_JSON " + JSON.stringify(record))
	print("TASK_0024AA_IDENTITY_SHIFT_BRIDGE_PASS")
	quit(0)
...
func _run() -> void:
	...
	if not loaded.ok:
		printerr("TASK_0024AA_LOAD_DETAIL " + JSON.stringify(loaded))
		return
...
func _require(condition: bool, label: String) -> void:
	if not condition:
		failures += 1
		printerr("TASK_0024AA_ASSERT_FAIL " + label)
