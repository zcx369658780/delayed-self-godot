...
func _run() -> void:
	...
	if failures > 0:
		printerr("TASK_0024T_V2_COMPLETE_FAIL failures=%d" % failures)
		quit(1)
	else:
		print("TASK_0024T_V2_COMPLETE_PASS")
		quit(0)
...
func _test_canonical_contract() -> void:
	var source := _fixture("key_lock").level.duplicate(true)
	...
	var normalized_order_invariant := ordered.ok and reordered.ok
	...
	var level := _fixture("integration").level
	...
	var latch_level := _fixture("latch_defer").level
	...
	var door_source := _fixture("key_lock").level.duplicate(true)
...
func _test_crate_and_key_contract() -> void:
	var crate_level := _fixture("crate_echo").level
	...
	var key_level := _fixture("key_lock").level
...
func _test_sensor_contract() -> void:
	var level := _fixture("sensor_barrier").level
	...
	var crate_level := _fixture("integration").level
...
func _test_barrier_and_latch_contract() -> void:
	...
	var vacated_state := latched.state.duplicate(true) if latched.ok else {}
...
func _test_malformed_contract() -> void:
	var source := _fixture("key_lock").level.duplicate(true)
	var variant := source.duplicate(true); variant["unknown_top"] = true
...
func _case(case_id: String, condition: bool) -> void:
	if not condition:
		failures += 1
		printerr("TASK_0024T_CASE_FAIL " + case_id)
