...
WORKER=TOOLS/"task_0024ax_case_worker.ps1"
CASES=(
 ("focused_s01","res://tests/levels/task_0024ax_s01_explicit_role_separation_validation.gd","TASK_0024AX_S01_EXPLICIT_ROLE_SEPARATION_PASS","RequireEmpty"),
...
def consume(path,run_id):
 ...
 expected={"schema_version":1,"task_identity":"0024AX","manifest_id":run_id,"status":"DECLARED","cases":[list(c) for c in CASES]}
 if manifest!=expected: raise RuntimeError("MANIFEST_CONTRACT_INVALID")
 consumed=path.with_suffix(path.suffix+".consumed.json")
 atomic(consumed,{"schema_version":1,"task_identity":"0024AX","manifest_id":run_id,"status":"CONSUMED","consumed_utc":datetime.now(timezone.utc).isoformat()},True)
def run(mode,kind,manifest):
 ...
 for ordinal,(name,script,marker,policy) in enumerate(selected,1):
  target=TOOLS/f"task_0024ax_{mode}_{run_id}_case_{ordinal:02d}_{name}.json"
  ...
  atomic(TOOLS/f"task_0024ax_{mode}_{run_id}_progress.json",{"schema_version":1,"task_identity":"0024AX","run_id":run_id,"status":"IN_PROGRESS","completed":journal})
  if done.returncode!=0: raise RuntimeError(f"WORKER_FAILED:{name}:{done.stderr}")
 result={"schema_version":1,"task_identity":"0024AX","run_id":run_id,"mode":mode,"kind":kind,"status":"PASS","cases":journal,"godot_final_count":godot_count(),"completed_utc":datetime.now(timezone.utc).isoformat()}
 result_path=TOOLS/f"task_0024ax_{mode}_{run_id}_result.json"; atomic(result_path,result,True)
