...
WORKER=TOOLS/"task_0024ayr_case_worker.ps1"
CASES=(
 ("focused_c03_retry","res://tests/levels/task_0024ayr_c03_crate_delayed_door_validation.gd","TASK_0024AYR_C03_NEXT_TURN_DOOR_PASS","RequireEmpty"),
...
def consume(path,run_id):
 ...
 expected={"schema_version":1,"task_identity":"0024AYR","manifest_id":run_id,"status":"DECLARED","cases":[list(c) for c in CASES]}
 if manifest!=expected: raise RuntimeError("MANIFEST_CONTRACT_INVALID")
 atomic(path.with_suffix(path.suffix+".consumed.json"),{"schema_version":1,"task_identity":"0024AYR","manifest_id":run_id,"status":"CONSUMED","consumed_utc":datetime.now(timezone.utc).isoformat()},True)
def run(mode,kind,manifest):
 ...
 if mode=="retained":
  ...
  data=json.loads(manifest.read_text(encoding="utf-8")); run_id=data["manifest_id"]; consume(manifest,run_id); selected=CASES
 if godot_count()!=0: raise RuntimeError("PREEXISTING_GODOT")
 journal=[]
 for ordinal,(name,script,marker,policy) in enumerate(selected,1):
  target=TOOLS/f"task_0024ayr_{mode}_{run_id}_case_{ordinal:02d}_{name}.json"
  ...
  atomic(TOOLS/f"task_0024ayr_{mode}_{run_id}_progress.json",{"schema_version":1,"task_identity":"0024AYR","run_id":run_id,"status":"IN_PROGRESS","completed":journal})
  if done.returncode!=0: raise RuntimeError(f"WORKER_FAILED:{name}:{done.stderr}")
 result={"schema_version":1,"task_identity":"0024AYR","run_id":run_id,"mode":mode,"kind":kind,"status":"PASS","cases":journal,"godot_final_count":godot_count(),"completed_utc":datetime.now(timezone.utc).isoformat()}
 result_path=TOOLS/f"task_0024ayr_{mode}_{run_id}_result.json"; atomic(result_path,result,True)
 print(compact({"status":"PASS","run_id":run_id,"result_path":str(result_path)}))
def main():
