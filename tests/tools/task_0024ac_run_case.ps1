...
$record | Add-Member -NotePropertyName candidate_sha256 -NotePropertyValue (Get-FileHash -LiteralPath (Join-Path $projectPath 'data\levels\lasting_echo_latch.json') -Algorithm SHA256).Hash.ToLowerInvariant()
[IO.File]::WriteAllText($output, ($record | ConvertTo-Json -Depth 100) + "`n", [Text.UTF8Encoding]::new($false))
Write-Output ("TASK_0024AC_CASE_RECORD_WRITTEN " + $output)
exit $childExit
