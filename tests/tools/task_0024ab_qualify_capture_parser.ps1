...
$root = 'D:\Delayed_Self'
$parser = Join-Path $root 'tests\tools\task_0024ab_validate_capture.ps1'
...
function Invoke-ParserProbe([string]$Name, [scriptblock]$Mutate, [string]$ExpectedError) {
    ...
    & $Mutate $lines
    $source.stdout = @($lines)
    $inputRelative = "tests/tools/task_0024ab_parser_probe_$($Name)_input.json"
    $resultRelative = "tests/tools/task_0024ab_parser_probe_$($Name)_result.json"
...
$metricsPrefix = 'TASK_0024AB_METRICS_JSON '
$probes = [Collections.Generic.List[object]]::new()
$probes.Add((Invoke-ParserProbe 'missing' { param($lines) for ($i=$lines.Count-1; $i -ge 0; $i--) { if ($lines[$i].StartsWith($metricsPrefix)) { $lines.RemoveAt($i) } } } 'count:TASK_0024AB_METRICS_JSON'))
$probes.Add((Invoke-ParserProbe 'duplicate' { param($lines) $line=@($lines|Where-Object{$_.StartsWith($metricsPrefix)})[0]; $lines.Add($line) } 'count:TASK_0024AB_METRICS_JSON'))
$probes.Add((Invoke-ParserProbe 'truncated' { param($lines) for($i=0;$i -lt $lines.Count;$i++){if($lines[$i].StartsWith($metricsPrefix)){$lines[$i]=$metricsPrefix+'{"status":"SOLVED"';break}} } 'non_json_or_truncated:TASK_0024AB_METRICS_JSON'))
$probes.Add((Invoke-ParserProbe 'non_json' { param($lines) for($i=0;$i -lt $lines.Count;$i++){if($lines[$i].StartsWith($metricsPrefix)){$lines[$i]=$metricsPrefix+'NOT_JSON';break}} } 'non_json_or_truncated:TASK_0024AB_METRICS_JSON'))
$probes.Add((Invoke-ParserProbe 'over_bound' { param($lines) for($i=0;$i -lt $lines.Count;$i++){if($lines[$i].StartsWith($metricsPrefix)){$lines[$i]=$lines[$i]+(' ' * 1700);break}} } 'over_bound:TASK_0024AB_METRICS_JSON'))

$failures = @($probes | Where-Object { -not $_.expected_rejection })
$summary = [ordered]@{ schema_version = 1; task = '0024AB'; status = if ($failures.Count -eq 0) { 'PASS' } else { 'FAIL' }; baseline_record = $BaselineRecordPath.Replace('\','/'); baseline_sha256 = (Get-FileHash -LiteralPath $baselineFile -Algorithm SHA256).Hash.ToLowerInvariant(); probe_count = $probes.Count; probes = @($probes); failures = @($failures) }
[IO.File]::WriteAllText($outputFile, ($summary | ConvertTo-Json -Depth 100) + "`n", $utf8)
Write-Output ("TASK_0024AB_CAPTURE_PARSER_" + $summary.status + " " + $outputFile)
