...
if (-not $process.Start()) {
    throw 'Failed to start accepted wrapper child process'
}
...
if ($stderrLines.Count -ne 0) {
    throw ("Wrapper child emitted process stderr: " + ($stderrLines -join ' | '))
}
if ($lines.Count -ne 1) {
    throw "Expected exactly one wrapper JSON line, observed $($lines.Count)"
}
...
[IO.File]::WriteAllText([IO.Path]::GetFullPath($OutputPath), $json + "`n", $utf8NoBom)
Write-Output ("TASK_0024Y_CASE_RECORD_WRITTEN " + [IO.Path]::GetFullPath($OutputPath))
exit $childExit
