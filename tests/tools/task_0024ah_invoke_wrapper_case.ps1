...
function Stop-Task0024AHCapture([string]$Reason) { throw "TASK_0024AH_WRAPPER_CAPTURE_FAIL:$Reason" }
function ConvertFrom-Task0024AHWrapperValues {
 ...
 if($nonEmpty.Count -ne 1){Stop-Task0024AHCapture "EXPECTED_ONE_NONEMPTY_VALUE:$($nonEmpty.Count)"}
 if($nonEmpty[0] -isnot [string]){Stop-Task0024AHCapture "RESULT_NOT_STRING:$($nonEmpty[0].GetType().FullName)"}
 $raw=[string]$nonEmpty[0]; $trimmed=$raw.Trim()
 if(-not $trimmed.StartsWith('{') -or -not $trimmed.EndsWith('}')){Stop-Task0024AHCapture 'RESULT_NOT_JSON_OBJECT'}
 try{$record=$raw|ConvertFrom-Json -Depth 100 -ErrorAction Stop}catch{Stop-Task0024AHCapture 'INVALID_JSON'}
 if($record -is [array] -or $record -isnot [pscustomobject]){Stop-Task0024AHCapture 'RESULT_NOT_OBJECT'}
 ...
 foreach($entry in $requirements.GetEnumerator()){
  $p=$record.PSObject.Properties[$entry.Key]; if($null -eq $p -or $null -eq $p.Value){Stop-Task0024AHCapture "MISSING_OR_NULL:$($entry.Key)"}; $v=$p.Value
  $ok=switch($entry.Value){'integer'{$v -is [int] -or $v -is [long]};'string'{$v -is [string]};'boolean'{$v -is [bool]};'array'{$v -is [array]}}
  if(-not $ok){Stop-Task0024AHCapture "WRONG_TYPE:$($entry.Key):$($entry.Value)"}
 }
 if([int]$record.schema_version -ne 2){Stop-Task0024AHCapture 'SCHEMA_VERSION_NOT_2'}
...
if($Library){return}
$wrapper=Join-Path $PSScriptRoot 'run_owned_godot_headless.ps1'; if(-not(Test-Path -LiteralPath $wrapper -PathType Leaf)){Stop-Task0024AHCapture 'WRAPPER_NOT_FOUND'}
...
$captured=ConvertFrom-Task0024AHWrapperValues -Values $values
